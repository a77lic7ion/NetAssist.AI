...
async def check_ollama() -> bool:
    ...
    if settings.llm_provider != PROVIDER_OLLAMA:
        return True
    success, _, _ = await test_provider_connection(PROVIDER_OLLAMA, settings.llm_base_url)
    return success
...
async def test_provider_connection(provider: str, base_url: str = None, api_key: str = None) -> Tuple[bool, str, List[str]]:
    """Test connectivity to the AI provider and return available models if possible"""
    try:
        if provider == PROVIDER_OLLAMA:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 200:
                    ...
                    return True, "Successfully connected to Ollama", models
            ...
            return True, "OpenAI configuration valid (local check)", models

        elif provider == PROVIDER_GEMINI:
            models = ["gemini-pro", "gemini-1.5-pro", "gemini-1.5-flash"]
            return True, f"Configuration valid for {provider}", models
